...
public class Camera3Pcontroller {
	...
	public class OrbitAroundAction extends AbstractInputAction {
		...
		public void performAction(float time, Event evt) {
			...
			{
				float m00 = (float) target.getWorldRotation().elementAt(0, 0);
				float m11 = (float) target.getWorldRotation().elementAt(1, 1);
				float m20 = (float) target.getWorldRotation().elementAt(2, 0);
				float m22 = (float) target.getWorldRotation().elementAt(2, 2);
				if (m20>0)
					cameraAzimuth = (float) Math.toDegrees(Math.acos(-(m00 + m11 + m22 -1)/2));
				else
					cameraAzimuth = (float) Math.toDegrees(Math.acos((m00 + m11 + m22 -1)/2))+180;
				new TurnRightAction(target).performAction(time,  evt);
			}
		...
		public void performAction(float time, Event evt) {
			...
			else
			{
				...
				if (m20>0)
					...
					cameraAzimuth = (float) Math.toDegrees(Math.acos((m00 + m11 + m22 -1)/2))+180;
				new TurnLeftAction(target).performAction(time,  evt);
			}
	...
	public class ZoomOutAction extends AbstractInputAction
	{
		public void performAction(float time, Event evt) {
			float zoom = 0;
			if (evt.getValue() < -0.2 ) { zoom=0.02f; }
			 else { if (evt.getValue() > 0.2 && cameraDistanceFromTarget>1) { zoom=-0.02f; }
			 else { zoom=0.0f; }
			 }
	...
	public void setCameraAzimuth(float theta)
	{
		cameraAzimuth = theta;
	}
}
